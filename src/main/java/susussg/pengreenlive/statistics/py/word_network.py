...
def generate_image(reviews):
    ...
    for review in reviews:
        nouns.extend([noun for noun in okt.nouns(review) if noun not in josa_list])
    ...
    for word, (x, y) in pos.items():
        plt.text(x, y, word, fontsize=G.nodes[word]['size']*10, ha='center', va='center', fontproperties=font_prop, color=random_colors[word])
    plt.axis('off')
    img_buf = io.BytesIO()
...
if __name__ == '__main__':
    ...
    img_base64 = generate_image(reviews)
